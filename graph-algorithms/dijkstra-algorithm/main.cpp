...
int main()
{
    ...
    vertexList[0]->addAdj(edge01.get());
    vertexList[0]->addAdj(edge04.get());
    vertexList[0]->addAdj(edge07.get());
    vertexList[1]->addAdj(edge13.get());
    vertexList[1]->addAdj(edge14.get());
    vertexList[1]->addAdj(edge17.get());
    vertexList[2]->addAdj(edge23.get());
    vertexList[2]->addAdj(edge26.get());
    vertexList[3]->addAdj(edge36.get());
    vertexList[4]->addAdj(edge45.get());
    vertexList[4]->addAdj(edge46.get());
    vertexList[4]->addAdj(edge47.get());
    vertexList[5]->addAdj(edge52.get());
    vertexList[5]->addAdj(edge56.get());
    vertexList[7]->addAdj(edge72.get());
    vertexList[7]->addAdj(edge75.get());

    DijkstraAlgorithm sp;
    sp.computePaths(vertexList[0]);

    auto path = sp.getPath(vertexList[6]);
    for (Vertex *v : path)
        std::cout << *v << ' ';
